...
export default function TeacherZoomManager({
  ...
  const fetchTeacherMeetings = async () => {
    try {
      const response = await fetch(`https://quran-acadamy-crm-backend-production.up.railway.app/api/meetings/teacher/${currentUser?.id}`);
      if (response.ok) {
  ...
  const handleEndClass = async (meetingId: string) => {
    try {
      setIsLoading(true);
      const response = await fetch(`https://quran-acadamy-crm-backend-production.up.railway.app/api/meetings/${meetingId}/end`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
  ...
  const handleSaveEdit = async () => {
    ...
    setIsLoading(true);
    try {
      const response = await fetch(`https://quran-acadamy-crm-backend-production.up.railway.app/api/meetings/${editingMeeting._id}`, {
        method: 'PUT',
        headers: {
          'Content-Type': 'application/json',
